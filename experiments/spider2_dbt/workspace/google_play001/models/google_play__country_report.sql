...
country_join as (
    ...
    select 
        ...
        coalesce(install_metrics.country, ratings.country, store_performance_metrics.country_region) as country_short,
    ...
    full outer join store_performance_metrics
        on coalesce(install_metrics.date_day, ratings.date_day) = store_performance_metrics.date_day
        and coalesce(install_metrics.source_relation, ratings.source_relation) = store_performance_metrics.source_relation
        and coalesce(install_metrics.package_name, ratings.package_name) = store_performance_metrics.package_name
        -- coalesce null countries otherwise they'll cause fanout with the full outer join
        and coalesce(install_metrics.country, ratings.country, 'null_country') = coalesce(store_performance_metrics.country_region, 'null_country')
), 
...
create_partitions as (
    ...
    {% for metric in rolling_metrics -%}
        , sum(case when {{ metric }} is null 
                then 0 else 1 end) over (partition by source_relation, country_short, package_name order by date_day asc rows unbounded preceding) as {{ metric | lower }}_partition
    {%- endfor %}
    from country_join
), 
...
fill_values as (
    ...
    select 
        ...
        , first_value( {{ metric }} ) over (
            partition by source_relation, {{ metric | lower }}_partition, country_short, package_name order by date_day asc rows between unbounded preceding and current row) as {{ metric }}
...
add_country_info as (
    ...
    left join country_codes
        on country_codes.country_code_alpha_2 = final.country_short
)
