...
-- NOTE: `GRANT USAGE ON DATABASE` alone grants zero access to the objects
-- inside — Snowflake requires explicit schema-level USAGE plus object-level
-- privileges. `GRANT ALL ON DATABASE` also does not cascade to schemas or
-- tables inside the clone (cloned tables retain the original creator as
-- owner; the granted role can traverse but not read/write). To make this
-- example actually work, grant at schema and object level too.
...
GRANT USAGE ON ALL SCHEMAS IN DATABASE RAW_DEV       TO ROLE DATA_PLATFORM_ADMIN;
GRANT USAGE ON ALL SCHEMAS IN DATABASE TRANSFORM_DEV TO ROLE DATA_PLATFORM_ADMIN;
GRANT USAGE ON ALL SCHEMAS IN DATABASE ANALYTICS_DEV TO ROLE DATA_PLATFORM_ADMIN;
GRANT SELECT ON ALL TABLES IN DATABASE RAW_DEV       TO ROLE DATA_PLATFORM_ADMIN;
GRANT SELECT ON ALL TABLES IN DATABASE TRANSFORM_DEV TO ROLE DATA_PLATFORM_ADMIN;
GRANT SELECT ON ALL TABLES IN DATABASE ANALYTICS_DEV TO ROLE DATA_PLATFORM_ADMIN;

-- dbt CI needs write access to ANALYTICS_DEV — schema + object grants required
GRANT USAGE ON DATABASE ANALYTICS_DEV TO ROLE TRANSFORM_ROLE;
GRANT USAGE, CREATE SCHEMA ON DATABASE ANALYTICS_DEV TO ROLE TRANSFORM_ROLE;
GRANT USAGE ON ALL SCHEMAS IN DATABASE ANALYTICS_DEV TO ROLE TRANSFORM_ROLE;
GRANT CREATE TABLE, CREATE VIEW ON ALL SCHEMAS IN DATABASE ANALYTICS_DEV TO ROLE TRANSFORM_ROLE;
GRANT SELECT, INSERT, UPDATE, DELETE, TRUNCATE ON ALL TABLES IN DATABASE ANALYTICS_DEV TO ROLE TRANSFORM_ROLE;
GRANT SELECT, INSERT, UPDATE, DELETE, TRUNCATE ON FUTURE TABLES IN DATABASE ANALYTICS_DEV TO ROLE TRANSFORM_ROLE;
-- Alternative for full CI autonomy: make TRANSFORM_ROLE own the ANALYTICS_DEV
-- schemas so it can freely DROP/RECREATE:
--   GRANT OWNERSHIP ON ALL SCHEMAS IN DATABASE ANALYTICS_DEV TO ROLE TRANSFORM_ROLE COPY CURRENT GRANTS;
