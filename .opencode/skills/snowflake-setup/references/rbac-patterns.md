...
Access roles (object-scoped):
  ...
  TRANSFORM_WRITE         → USAGE + CREATE TABLE + SELECT/INSERT/UPDATE/DELETE
                            + FUTURE variants on TRANSFORM.*
  ANALYTICS_WRITE         → USAGE + CREATE TABLE + SELECT/INSERT/UPDATE/DELETE
                            + FUTURE variants on ANALYTICS.*
```
