...
describe.skipIf(!DOCKER && !MONGODB_USE_CI)("MongoDB Driver E2E", () => {
  ...
  describe("Connection", () => {
    ...
    test("close() cleans up and prevents further operations", async () => {
      ...
      await expect(
        conn.execute(cmd({ command: "find", database: "testdb", collection: "users" })),
      ).rejects.toThrow()
    })
  })
  ...
  describe("CRUD Operations", () => {
    ...
    test("updateOne — modifies a single document", async () => {
      ...
      // Verify the update
      const verify = await connector.execute(cmd({
        command: "find",
        database: "testdb",
        collection: "users",
        filter: { name: "alice" },
    ...
    test("deleteOne — removes a single document", async () => {
      ...
      // Verify deletion
      const count = await connector.execute(cmd({
        command: "countDocuments",
        database: "testdb",
        collection: "users",
      }))
  ...
  describe("Error Handling", () => {
    test("rejects invalid JSON query", async () => {
      await expect(
        connector.execute("not valid json {{{"),
      ).rejects.toThrow(/Invalid MQL query/)
    })

    test("rejects query without command field", async () => {
      await expect(
        connector.execute(JSON.stringify({ database: "testdb", collection: "users" })),
      ).rejects.toThrow(/must include a 'command' field/)
    })
  ...
  describe("Adversarial Tests", () => {
    ...
    test("handles duplicate key error (unique index violation)", async () => {
      ...
      const existingEmails = await connector.execute(cmd({
        command: "find",
        database: "testdb",
        collection: "users",
    ...
    test("handles array values in documents", async () => {
      ...
      const filtered = await connector.execute(cmd({
        command: "find",
        database: "testdb",
        collection: "arrays",
        filter: { tags: "y" },
    ...
    test("handles boolean edge cases", async () => {
      ...
      const falseResult = await connector.execute(cmd({
        command: "find",
        database: "testdb",
        collection: "booleans",
        filter: { flag: false },
