...
// Requirement 5 gave the disclosure ONE definition, which removed drift between the TUI notice and
// the HTTP disclosure route. It did nothing for the other consistency axis: the notice versus the
// fuller "Data handling" note in docs/docs/configure/providers.md. That axis had no mechanism
// except a comment saying "keep in sync" — and comments saying that had already failed three times
// in this feature. This file is the mechanism.
//
// The notice is deliberately a short summary, so it is NOT required to repeat everything the docs
// say (the per-installation identifier detail lives in docs only, per #1268). What it must never do
// is drop or weaken a *core* data term, because registration no longer waits for it to be read —
// it's the one text a user is shown, not a gate they pass through.

const DOCS = path.join(import.meta.dir, "../../../../docs/docs/configure/providers.md")

/** The claims the disclosure notice must carry, whatever the wording. */
const REQUIRED = [
...
describe("Altimate Base disclosure notice", () => {
  test("carries every core data term", () => {
    for (const claim of REQUIRED) {
      expect(
        claim.pattern.test(ALTIMATE_BASE_DISCLOSURE),
        `the disclosure notice no longer states: ${claim.name}`,
      ).toBe(true)
    }
  })
