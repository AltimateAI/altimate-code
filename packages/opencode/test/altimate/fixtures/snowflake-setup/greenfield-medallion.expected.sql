...
CREATE STORAGE INTEGRATION IF NOT EXISTS s3_bronze_integration
  ...
  STORAGE_PROVIDER = 'S3'
  ENABLED = TRUE
  STORAGE_AWS_ROLE_ARN = '<STORAGE_AWS_ROLE_ARN>'                 -- REPLACE BEFORE RUNNING
  STORAGE_ALLOWED_LOCATIONS = ('<S3_RAW_BUCKET_URL>');             -- REPLACE BEFORE RUNNING, e.g. 's3://your-company-raw-data/'

-- After creation, run this and copy STORAGE_AWS_IAM_USER_ARN + STORAGE_AWS_EXTERNAL_ID
-- into your AWS IAM role's trust policy (see "Manual (outside Snowflake)" section below):
...
CREATE USER IF NOT EXISTS dbt_service
  ...
  RSA_PUBLIC_KEY = '<DBT_SERVICE_PUBLIC_KEY>'                     -- REPLACE BEFORE RUNNING
...
CREATE STAGE IF NOT EXISTS BRONZE.APP.s3_stage
  STORAGE_INTEGRATION = s3_bronze_integration
  URL = '<S3_RAW_BUCKET_URL>APP/'                                 -- REPLACE BEFORE RUNNING
  FILE_FORMAT = (FORMAT_NAME = BRONZE.APP.parquet_standard);
...
CREATE TABLE IF NOT EXISTS BRONZE.APP.CUSTOMERS (
  customer_id     VARCHAR,
  email           VARCHAR,
  first_name      VARCHAR,
  last_name       VARCHAR,
...
CREATE PIPE IF NOT EXISTS BRONZE.APP.customers_pipe
  AUTO_INGEST = TRUE
  AS
  COPY INTO BRONZE.APP.CUSTOMERS (customer_id, email, first_name, last_name, created_at)
  FROM @BRONZE.APP.s3_stage/customers/
  FILE_FORMAT = (FORMAT_NAME = BRONZE.APP.parquet_standard)
  ON_ERROR = 'CONTINUE';
