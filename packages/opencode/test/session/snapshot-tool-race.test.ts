  ...
  () =>
    provideTmpdirServerLegacy(
      Effect.fnUntraced(function* ({ dir, llm }) {
        ...
        // Use bash tool (always registered) to create a file. `description` is a
        // required bash tool argument — omitting it makes the mocked tool call fail
        // schema validation before it ever executes, which was masking the actual
        // snapshot-race scenario this test exists to guard.
        const command = `echo 'snapshot race test content' > ${path.join(dir, "race-test.txt")}`
        yield* llm.toolMatch((hit) => JSON.stringify(hit.body).includes("create the file"), "bash", {
          command,
          description: "Creates race-test.txt",
        })
