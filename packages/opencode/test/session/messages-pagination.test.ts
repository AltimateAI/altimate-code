  ...
  it.instance("returns an Effect", () =>
    withSession(({ sessionID }) =>
      Effect.gen(function* () {
        ...
        const result = Array.from(MessageV2.stream(sessionID))
...
describe("MessageV2.filterCompacted", () => {
  ...
  it.instance("fork remaps compaction tail_start_id for filterCompacted", () =>
    Effect.gen(function* () {
      ...
      const tailPart = childFiltered
        .flatMap((m) => m.parts)
        .find((p) => p.type === "compaction") as SessionV1.CompactionPart | undefined
  ...
  test("works with array input", () => {
    ...
    const result = MessageV2.filterCompacted(items as unknown as MessageV2.WithParts[])
    expect(result).toHaveLength(1)
  ...
  it.instance("filterCompacted of full stream returns same as Array.from when no compaction", () =>
    withSession(({ sessionID }) =>
      Effect.gen(function* () {
        ...
        const stream = Array.from(MessageV2.stream(sessionID))
