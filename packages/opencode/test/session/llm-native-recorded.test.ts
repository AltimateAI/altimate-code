...
const zenURL = (connection: string) => `https://console.altimate.ai/proxy/connections/${connection}/v1`
...
// a missing gpt-5.5 fixture, and the proxy/Anthropic recorded HTTP requests no longer match, so replay falls through
// to live provider URLs before the recorded tool loop can settle.
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      ...
      JSON.stringify({ $schema: "https://altimate.ai/config.json", ...scenario.config(model) }),
    ),
  )
