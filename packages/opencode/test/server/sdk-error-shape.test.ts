...
describe("v2 SDK error shape", () => {
  it.live("404 with NamedError body throws a real Error carrying the server message", () =>
    Effect.gen(function* () {
  ...
  it.live("400 schema rejection: SDK extracts the field-level reason from the NamedError body", () =>
    ...
    Effect.gen(function* () {
      const directory = yield* tmpdirScoped({ config: { formatter: false, lsp: false } })
      const sdk = yield* client(directory)
