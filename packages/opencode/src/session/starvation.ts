    ...
    onToolCall(input: { tool: string; input: unknown }): CallResult {
      ...
      const command =
        input.input && typeof input.input === "object" && typeof (input.input as any).command === "string"
          ...
          : undefined
      const toolIdentity = input.tool.replace(/([a-z0-9])([A-Z])/g, "$1 $2").replace(/[_-]+/g, " ")
      const polling = pollingRegex.test(toolIdentity) || (command !== undefined && pollingRegex.test(command))
...
function sameConfig(left: ResolvedConfig, right: ResolvedConfig): boolean {
  return (
    ...
    left.maxTurnsWithoutMutation === right.maxTurnsWithoutMutation &&
    left.repeatSignatureThreshold === right.repeatSignatureThreshold &&
    left.doomLoopThreshold === right.doomLoopThreshold &&
    left.pollingThresholdMultiplier === right.pollingThresholdMultiplier &&
    left.pollingPattern === right.pollingPattern &&
    left.exemptAgents.length === right.exemptAgents.length &&
    left.exemptAgents.every((value, index) => value === right.exemptAgents[index]) &&
    left.generatedPathPatterns.length === right.generatedPathPatterns.length &&
    left.generatedPathPatterns.every((value, index) => value === right.generatedPathPatterns[index])
  )
}

export function forSession(sessionID: string, config: ResolvedConfig): Tracker {
  let tracker = trackers.get(sessionID)
  if (!tracker || !sameConfig(tracker.config, config)) {
    // Replacing a live session's tracker must not count as an extra cache
    // entry (or evict an unrelated session at capacity).
    if (tracker) trackers.delete(sessionID)
...
export * as SessionStarvation from "./starvation"
