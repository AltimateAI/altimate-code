...
// altimate_change start — upstream_fix: git auto-merge silently dropped this import when it took
// theirs' unrelated import-line change; SessionExecution.noopLayer below still needs it.
...
// NOTE: dropped the fork's standalone `export function* listGlobal(...)` that used to live here.
// It referenced an undefined `runtime` symbol (never imported in this file — already dead before
// this merge), had zero external callers (the only `Session.listGlobal(...)` call site imports
// `Session` from the legacy `session/index.ts` facade, which has its own independent copy), and
// was byte-identical to the pre-fork base. Upstream's Effect-native `listGlobal` service method
