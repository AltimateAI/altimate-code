...
// process-free logic that deserves direct tests. Keeping it in the namespace forced a choice
// between two bad options: exporting an implementation detail through `export * as` (which
// packages/opencode/AGENTS.md prohibits), or driving it through `search()` with a stub `rg` on
// PATH — and the binary lookup there is memoised per process, so such a stub leaks into every
// later test file in the same `bun test` run and breaks unrelated suites.
//
// It mirrors packages/core/src/ripgrep.ts, but the two are not identical by design. The core
// parser streams; this one buffers all of stdout before splitting, and hands its records straight
// to the `/find` response. Both cap retained text, skip records they cannot use, and report the
// skips once per search rather than once per record.
import z from "zod"
