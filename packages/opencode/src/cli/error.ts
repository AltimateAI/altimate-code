...
export function FormatError(input: unknown): string | undefined {
  ...
  if (NamedError.hasName(input, "MCPFailed")) {
    ...
    return `MCP server "${data}" failed. Note, altimate-code does not support MCP authentication yet.`
    // altimate_change end
  }
  ...
  if (remoteAuth) {
    ...
    return [
      ...
      // altimate_change start — user-facing auth command branding
      ...(url ? [`Run \`altimate auth login ${url}\` to re-authenticate.`] : []),
      // altimate_change end
    ].join("\n")
  }
