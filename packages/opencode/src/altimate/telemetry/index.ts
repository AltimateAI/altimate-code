...
export namespace Telemetry {
  ...
  export function maskString(s: string): string {
    return s
      ...
      .replace(/(^|[\s"'`=(,[{])(?:(?:gs|s3[an]?|abfss?|wasbs?|adl|dbfs|hdfs):\/\/|file:\/{1,3})[^\s'"`)\]},]*(?:(?: [\w.@+()-]+)+\/[^\s'"`)\]},]*)*(?: [\w.@+()-]+\.[A-Za-z0-9]{1,8})?(?:(?<!\.[A-Za-z0-9]{1,8}) [\w-]+(?=$|[.,;:)\]}!?]))?/gi, "$1<path>")
      .replace(/(^|[\s"'`=(,[{])[A-Za-z]:[\\\/]Users[\\\/][^\s'"`)\]},]*(?:(?: [\w.@+()-]+)+[\\\/][^\s'"`)\]},]*)*(?: [\w.@+()-]+\.[A-Za-z0-9]{1,8})?(?:(?<!\.[A-Za-z0-9]{1,8}) [\w-]+)?/gi, "$1<path>")
      .replace(/(^|[\s"'`=(,[{])(?:[A-Za-z]:[\\\/]|\\\\)[^\s'"`)\]},]+(?:(?: [\w.@+()-]+)+[\\\/][^\s'"`)\]},]*)*(?: [\w.@+()-]+\.[A-Za-z0-9]{1,8})?(?:(?<!\.[A-Za-z0-9]{1,8}) [\w-]+(?=$|[.,;:)\]}!?]))?/g, "$1<path>")
      .replace(/(^|[\s"'`=(,[{])\/(?:Users|home)\/[^\s'"`)\]},]*(?:(?: [\w.@+()-]+)+\/[^\s'"`)\]},]*)*(?: [\w.@+()-]+\.[A-Za-z0-9]{1,8})?(?:(?<!\.[A-Za-z0-9]{1,8}) [\w-]+)?/gi, "$1<path>")
      .replace(/(^|[\s"'`=(,[{])\/(?:[\w.@+-]+\/)+[^\s'"`)\]},]*(?:(?: [\w.@+()-]+)+\/[^\s'"`)\]},]*)*(?: [\w.@+()-]+\.[A-Za-z0-9]{1,8})?(?:(?<!\.[A-Za-z0-9]{1,8}) [\w-]+(?=$|[.,;:)\]}!?]))?/g, "$1<path>")
      .replace(/(^|[\s"'`=(,[{])~\/[^\s'"`)\]},]*(?:(?: [\w.@+()-]+)+\/[^\s'"`)\]},]*)*(?: [\w.@+()-]+\.[A-Za-z0-9]{1,8})?(?:(?<!\.[A-Za-z0-9]{1,8}) [\w-]+(?=$|[.,;:)\]}!?]))?/g, "$1<path>")
      // altimate_change end
      .replace(/'(?:[^'\\]|\\.)*'/g, "?")
      .replace(/"(?:[^"\\]|\\.)*"/g, "?")
      .replace(/\s+/g, " ")
