...
// altimate_change upstream_fix: the fresh schema is shared by core and the
// legacy compatibility DB path during the v1.17.9 transition, so CREATEs must
// be idempotent when both paths observe the same sqlite file.
export default {
  up(tx) {
