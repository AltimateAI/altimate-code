...
describe("public event manifest", () => {
  test("owns the complete public event surface", () => {
    // 61/89: upstream's 55/85 + the fork's session.status/idle/phase in ServerDefinitions
    // (the /api/event union — see the altimate_change in event-manifest.ts) and merge-era
    // definition additions; Latest/Durable pins updated below to match.
  ...
  test("uses canonical definitions for current public events", () => {
    ...
    // Position-independent (merge-era definitions shift slice offsets): assert identity
    // through the Latest map instead of a hardcoded slice window.
