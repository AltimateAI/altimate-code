...
fn configure_display_backend() -> Option<String> {
    use altimate_code_lib::linux_windowing::{Backend, SessionEnv, select_backend};
    use std::env;
    ...
    let prefer_wayland = altimate_code_lib::linux_display::read_wayland().unwrap_or(false);
