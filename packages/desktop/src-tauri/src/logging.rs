...
pub fn init(log_dir: &Path) -> WorkerGuard {
    ...
    let filename = format!("altimate-code-desktop_{timestamp}.log");
    ...
    let filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        if cfg!(debug_assertions) {
            EnvFilter::new("altimate_code_lib=debug,altimate_code_desktop=debug,sidecar=debug")
        } else {
            EnvFilter::new("altimate_code_lib=info,altimate_code_desktop=info,sidecar=info")
        }
    });
