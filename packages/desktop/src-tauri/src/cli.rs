...
// altimate_change start — our `install` script installs `altimate` into ~/.altimate/bin
// (APP=altimate, INSTALL_DIR=$HOME/.altimate/bin), not ~/.opencode/bin/opencode.
const CLI_INSTALL_DIR: &str = ".altimate/bin";
const CLI_BINARY_NAME: &str = "altimate";
// altimate_change end
...
pub fn install_cli(app: tauri::AppHandle) -> Result<String, String> {
    ...
    let temp_script = std::env::temp_dir().join("altimate-install.sh");
...
pub fn spawn_command(
    ...
    let mut cmd = if cfg!(windows) {
        if is_wsl_enabled(app) {
            ...
            // altimate_change start — install OUR CLI in WSL: altimate binary in
            // ~/.altimate/bin via the Altimate installer (not opencode.ai/install).
            let mut script = vec![
                "set -e".to_string(),
                "BIN=\"$HOME/.altimate/bin/altimate\"".to_string(),
                "if [ ! -x \"$BIN\" ]; then".to_string(),
                format!(
                    "  curl -fsSL https://www.altimate.sh/install | bash -s -- --version {} --no-modify-path",
