...
export const SqlGuardCorrectTool = Tool.define("sqlguard_correct", {
  description:
    "Iteratively correct SQL using a propose-verify-refine loop via the Rust-based sqlguard engine. More thorough than fix — applies multiple correction rounds to produce valid SQL.",
  parameters: z.object({
  ...
  async execute(args, ctx) {
    try {
      const result = await Bridge.call("sqlguard.correct", {
        sql: args.sql,
