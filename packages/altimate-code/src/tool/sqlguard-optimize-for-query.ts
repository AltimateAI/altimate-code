...
export const SqlGuardOptimizeForQueryTool = Tool.define("sqlguard_optimize_for_query", {
  description:
    "Prune schema to only tables and columns relevant to a specific query using the Rust-based sqlguard engine. Reduces context size for LLM prompts.",
  parameters: z.object({
  ...
  async execute(args, ctx) {
    try {
      const result = await Bridge.call("sqlguard.optimize_for_query", {
        sql: args.sql,
