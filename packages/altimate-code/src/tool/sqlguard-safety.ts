...
export const SqlGuardSafetyTool = Tool.define("sqlguard_safety", {
  description:
    "Scan SQL for injection patterns, dangerous statements (DROP, TRUNCATE), and security threats. Uses the Rust-based sqlguard safety engine.",
  parameters: z.object({
    sql: z.string().describe("SQL query to scan"),
  }),
  async execute(args, ctx) {
    try {
      const result = await Bridge.call("sqlguard.safety", { sql: args.sql })
