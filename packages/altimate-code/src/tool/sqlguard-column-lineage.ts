...
export const SqlGuardColumnLineageTool = Tool.define("sqlguard_column_lineage", {
  description:
    "Trace schema-aware column lineage using the Rust-based sqlguard engine. Maps how columns flow through a query from source tables to output. Requires sqlguard.init() with API key.",
  parameters: z.object({
  ...
  async execute(args, ctx) {
    try {
      const result = await Bridge.call("sqlguard.column_lineage", {
        sql: args.sql,
