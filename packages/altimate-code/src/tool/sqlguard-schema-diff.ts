...
export const SqlGuardSchemaDiffTool = Tool.define("sqlguard_schema_diff", {
  description:
    "Diff two schemas and detect breaking changes using the Rust-based sqlguard engine. Compares old vs new schema files and identifies added, removed, and modified tables/columns.",
  parameters: z.object({
    schema1_path: z.string().describe("Path to the old/baseline schema file"),
    schema2_path: z.string().describe("Path to the new/changed schema file"),
  }),
  async execute(args, ctx) {
    try {
      const result = await Bridge.call("sqlguard.schema_diff", {
