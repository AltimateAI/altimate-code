...
export const SqlGuardCompleteTool = Tool.define("sqlguard_complete", {
  description:
    "Get cursor-aware SQL completion suggestions using the Rust-based sqlguard engine. Returns table names, column names, functions, and keywords relevant to the cursor position.",
  parameters: z.object({
  ...
  async execute(args, ctx) {
    try {
      const result = await Bridge.call("sqlguard.complete", {
        sql: args.sql,
