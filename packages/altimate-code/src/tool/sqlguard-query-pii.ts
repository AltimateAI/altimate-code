...
export const SqlGuardQueryPiiTool = Tool.define("sqlguard_query_pii", {
  description:
    "Analyze query-level PII exposure using the Rust-based sqlguard engine. Checks if a SQL query accesses columns classified as PII and reports the exposure risk.",
  parameters: z.object({
  ...
  async execute(args, ctx) {
    try {
      const result = await Bridge.call("sqlguard.query_pii", {
        sql: args.sql,
