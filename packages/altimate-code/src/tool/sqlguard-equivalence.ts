...
export const SqlGuardEquivalenceTool = Tool.define("sqlguard_equivalence", {
  description:
    "Check semantic equivalence of two SQL queries using the Rust-based sqlguard engine. Determines if two queries produce the same result set regardless of syntactic differences.",
  parameters: z.object({
  ...
  async execute(args, ctx) {
    try {
      const result = await Bridge.call("sqlguard.equivalence", {
        sql1: args.sql1,
        sql2: args.sql2,
