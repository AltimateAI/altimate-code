...
export const SqlGuardFixTool = Tool.define("sqlguard_fix", {
  description:
    "Auto-fix SQL errors using the Rust-based sqlguard engine. Uses fuzzy matching and iterative re-validation to correct syntax errors, typos, and schema reference issues.",
  parameters: z.object({
  ...
  async execute(args, ctx) {
    try {
      const result = await Bridge.call("sqlguard.fix", {
        sql: args.sql,
