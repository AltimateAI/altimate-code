...
export const SqlGuardLintTool = Tool.define("sqlguard_lint", {
  description:
    "Lint SQL for anti-patterns using the Rust-based sqlguard engine. Catches issues like NULL comparisons, implicit casts, unused CTEs, and dialect-specific problems.",
  parameters: z.object({
  ...
  async execute(args, ctx) {
    try {
      const result = await Bridge.call("sqlguard.lint", {
        sql: args.sql,
