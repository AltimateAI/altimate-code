...
export const SqlGuardCompareTool = Tool.define("sqlguard_compare", {
  description:
    "Structurally compare two SQL queries using the Rust-based sqlguard engine. Identifies differences in table references, join conditions, filters, projections, and aggregations.",
  parameters: z.object({
  ...
  async execute(args, ctx) {
    try {
      const result = await Bridge.call("sqlguard.compare", {
