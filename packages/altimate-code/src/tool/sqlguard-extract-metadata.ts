...
export const SqlGuardExtractMetadataTool = Tool.define("sqlguard_extract_metadata", {
  description:
    "Extract metadata from SQL using the Rust-based sqlguard engine. Identifies tables, columns, functions, CTEs, and other structural elements referenced in a query.",
  parameters: z.object({
  ...
  async execute(args, ctx) {
    try {
      const result = await Bridge.call("sqlguard.metadata", {
        sql: args.sql,
