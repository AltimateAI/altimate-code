...
export const SqlGuardIntrospectionSqlTool = Tool.define("sqlguard_introspection_sql", {
  description:
    "Generate INFORMATION_SCHEMA introspection queries for a given database type using the Rust-based sqlguard engine. Supports postgres, bigquery, snowflake, mysql, mssql, redshift.",
  parameters: z.object({
  ...
  async execute(args, ctx) {
    try {
      const result = await Bridge.call("sqlguard.introspection_sql", {
