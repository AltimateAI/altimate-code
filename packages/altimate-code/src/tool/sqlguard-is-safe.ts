...
export const SqlGuardIsSafeTool = Tool.define("sqlguard_is_safe", {
  description:
    "Quick boolean safety check for SQL using the Rust-based sqlguard engine. Returns true/false indicating whether the SQL is safe to execute (no injection, no destructive operations).",
  parameters: z.object({
    sql: z.string().describe("SQL query to check"),
  }),
  async execute(args, ctx) {
    try {
      const result = await Bridge.call("sqlguard.is_safe", {
        sql: args.sql,
      })
