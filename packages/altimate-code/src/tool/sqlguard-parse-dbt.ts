...
export const SqlGuardParseDbtTool = Tool.define("sqlguard_parse_dbt", {
  description:
    "Parse a dbt project directory using the Rust-based sqlguard engine. Extracts models, sources, tests, and project structure for analysis.",
  parameters: z.object({
    project_dir: z.string().describe("Path to the dbt project directory"),
  }),
  async execute(args, ctx) {
    try {
      const result = await Bridge.call("sqlguard.parse_dbt", {
