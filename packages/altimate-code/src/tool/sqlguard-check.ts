...
export const SqlGuardCheckTool = Tool.define("sqlguard_check", {
  description:
    "Run full analysis pipeline: validate + lint + safety scan + PII check using the Rust-based sqlguard engine. Single call for comprehensive SQL analysis.",
  parameters: z.object({
  ...
  async execute(args, ctx) {
    try {
      const result = await Bridge.call("sqlguard.check", {
        sql: args.sql,
