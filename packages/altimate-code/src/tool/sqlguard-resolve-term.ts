...
export const SqlGuardResolveTermTool = Tool.define("sqlguard_resolve_term", {
  description:
    "Resolve a business glossary term to schema elements using fuzzy matching via the Rust-based sqlguard engine. Maps human-readable terms like 'revenue' or 'customer' to actual table/column names.",
  parameters: z.object({
  ...
  async execute(args, ctx) {
    try {
      const result = await Bridge.call("sqlguard.resolve_term", {
        term: args.term,
