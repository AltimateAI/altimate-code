...
function globalFiles() {
  ...
  if (Flag.ALTIMATE_CLI_CONFIG_DIR) {
    files.push(path.join(Flag.ALTIMATE_CLI_CONFIG_DIR, "AGENTS.md"))
  }
  files.push(path.join(Global.Path.config, "AGENTS.md"))
  if (!Flag.ALTIMATE_CLI_DISABLE_CLAUDE_CODE_PROMPT) {
...
async function resolveRelative(instruction: string): Promise<string[]> {
  ...
  if (!Flag.ALTIMATE_CLI_CONFIG_DIR) {
    log.warn(
      `Skipping relative instruction "${instruction}" - no ALTIMATE_CLI_CONFIG_DIR set while project config is disabled`,
    )
    return []
  }
  return Filesystem.globUp(instruction, Flag.ALTIMATE_CLI_CONFIG_DIR, Flag.ALTIMATE_CLI_CONFIG_DIR).catch(() => [])
}
