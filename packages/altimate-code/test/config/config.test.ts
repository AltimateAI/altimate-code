  ...
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://altimate-code.sh/config.json",
        model: "test/model",
        username: "testuser",
      })
    },
...
test("loads config from .altimate-code directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const altimateDir = path.join(dir, ".altimate-code")
      await fs.mkdir(altimateDir, { recursive: true })
      const agentDir = path.join(altimateDir, "agent")
...
test("loads agents from .altimate-code/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const agentsDir = path.join(altimateDir, "agents")
...
test("loads commands from .altimate-code/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const commandDir = path.join(altimateDir, "command")
...
test("loads commands from .altimate-code/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const commandsDir = path.join(altimateDir, "commands")
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(dir, "altimate-code.json"),
        JSON.stringify({ $schema: "https://altimate-code.sh/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .altimate-code config
      const projectDir = path.join(dir, "project")
      const altimateDir = path.join(projectDir, ".altimate-code")
      await fs.mkdir(altimateDir, { recursive: true })
      ...
      // Local .altimate-code config with different plugins
      await Filesystem.write(
        path.join(altimateDir, "altimate-code.json"),
        JSON.stringify({
          $schema: "https://altimate-code.sh/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .altimate-code config with some overlapping plugins
      await Filesystem.write(
        path.join(altimateDir, "altimate-code.json"),
        JSON.stringify({
          $schema: "https://altimate-code.sh/config.json",
...
test("local .altimate-code config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .altimate-code directory config enables it
      const altimateDir = path.join(dir, ".altimate-code")
      await fs.mkdir(altimateDir, { recursive: true })
      await Filesystem.write(
        path.join(altimateDir, "altimate-code.json"),
...
describe("getPluginName", () => {
  ...
  test("extracts name from npm package with version", () => {
    expect(Config.getPluginName("oh-my-altimate@2.4.3")).toBe("oh-my-altimate")
  ...
  test("extracts name from scoped npm package", () => {
    ...
    expect(Config.getPluginName("@altimate/plugin@2.0.0")).toBe("@altimate/plugin")
  })
...
describe("deduplicatePlugins", () => {
  ...
  test("prefers local file over npm package with same name", () => {
    const plugins = ["oh-my-altimate@2.4.3", "file:///project/.altimate-code/plugin/oh-my-altimate.js"]

    const result = Config.deduplicatePlugins(plugins)

    expect(result.length).toBe(1)
    expect(result[0]).toBe("file:///project/.altimate-code/plugin/oh-my-altimate.js")
  })
  ...
  test("local plugin directory overrides global altimate-code.json plugin", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const altimateDir = path.join(projectDir, ".altimate-code")
        const pluginDir = path.join(altimateDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("ALTIMATE_CLI_DISABLE_PROJECT_CONFIG", () => {
  ...
  test("skips project .altimate-code/ directories when flag is set", async () => {
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .altimate-code directory with a command
          const altimateDir = path.join(dir, ".altimate-code", "command")
          await fs.mkdir(altimateDir, { recursive: true })
          await Filesystem.write(path.join(altimateDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          // Project .altimate-code should NOT be in directories list
