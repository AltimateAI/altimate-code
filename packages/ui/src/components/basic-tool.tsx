...
export function GenericTool(props: {
  ...
  // altimate_change — receive the tool output (already passed by the caller) so
  // custom/data tools render their full result, not just a 3-arg summary.
  output?: unknown
}) {
  const outputText = () => {
  ...
  return (
    ...
    >
      {/* altimate_change — full output body for custom tools (collapsible).
          Pass children ONLY when there's output, so output-less tools don't get
          an empty disclosure affordance (BasicTool keys the chevron off `children`). */}
      {hasOutput() && (
        <pre
          data-slot="generic-tool-output"
          ...
          style={{
            ...
            padding: "8px 10px",
            "border-radius": "6px",
            background: "var(--color-background-element, rgba(127, 127, 127, 0.08))",
          }}
        >
          {outputText()}
        </pre>
