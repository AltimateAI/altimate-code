---
title: Providers
description: Using any LLM provider in Altimate Code.
---

import config from "../../../config.mjs"
export const console = config.console

Altimate Code uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your Altimate Code config.
...
You can customize the providers through the `provider` section in your Altimate Code
config.
...
Altimate Code Zen is a list of models provided by the Altimate Code team that have been
tested and verified to work well with Altimate Code. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with Altimate Code Zen.
:::

1. Run the `/connect` command in the TUI, select `Altimate Code Zen`, and head to [altimate.ai/auth](https://altimate.ai/zen).
...
It works like any other provider in Altimate Code and is completely optional to use.

---

## Altimate Code Go

Altimate Code Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the Altimate Code team that have been
tested and verified to work well with Altimate Code.

1. Run the `/connect` command in the TUI, select `Altimate Code Go`, and head to [altimate.ai/auth](https://altimate.ai/zen).
...
To use Amazon Bedrock with Altimate Code:
...
:::info
Using your Claude Pro/Max subscription in Altimate Code is not officially supported by [Anthropic](https://anthropic.com).
:::
...
2. Choose your authentication method:
   ...
   3. Name: `Altimate Code`, Scopes: `api`
...
Altimate Code uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock Altimate Code
...
To use Google Vertex AI with Altimate Code:
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin to automatically log each Altimate Code conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each Altimate Code conversation listed as a separate session.
...
:::tip
Ollama can automatically configure itself for Altimate Code. See the [Ollama integration docs](https://docs.ollama.com/integrations/opencode) for details.
:::
...
To use Ollama Cloud with Altimate Code:
...
3. Copy the API key for use in Altimate Code.
...
6. **Important**: Before using cloud models in Altimate Code, you must pull the model information locally:
...
Altimate Code Zen is a list of tested and verified models provided by the Altimate Code team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>Altimate Code Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **Altimate Code Zen**.

   ```txt
   /connect
   ```

3. Enter your Altimate Code API key.
...
The `limit` fields allow Altimate Code to understand how much context you have left. Standard providers pull these from models.dev automatically.
