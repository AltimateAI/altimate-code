...
// connectable. For non-OpenCode providers we treat "registered" AS
// connected — custom `altimate-code.json` entries, BYOK entries with no
// per-token cost metadata, and self-hosted deployments legitimately omit
// the `cost` field, and we should not force them through the welcome
// picker on upgrade. For OpenCode specifically, `cost.input === 0` is a
// deliberate marker of the free tier (no Zen key), which does NOT count
// as connected — a nonzero (real) cost is what signals a paid Zen key.
