...
class SchemaCache:
    ...
    def index_warehouse(
        ...
        self._conn.execute("DELETE FROM columns_cache WHERE warehouse = ?", (warehouse_name,))
            ...
            for table_info in tables:
                ...
                self._conn.execute(
                    """INSERT OR REPLACE INTO tables_cache
                       ...
                       VALUES (?, ?, ?, ?, ?, ?)""",
                    (warehouse_name, database_name, schema_name, tname, ttype, search_text),
                )
                ...
                for col in columns:
                    ...
                    self._conn.execute(
                        """INSERT OR REPLACE INTO columns_cache
                           ...
                           VALUES (?, ?, ?, ?, ?, ?, ?, ?)""",
                        (warehouse_name, database_name, schema_name, tname,
                         cname, ctype, cnull, col_search),
                    )
        ...
        self._conn.execute(
            """INSERT OR REPLACE INTO warehouses
               ...
               VALUES (?, ?, ?, ?, ?, ?, ?)""",
            (warehouse_name, warehouse_type, now,
             1 if database_name else 0, total_schemas, total_tables, total_columns),
        )
        self._conn.commit()
    ...
    def search(
        ...
        search_condition = " AND ".join(where_clauses)
        ...
        for row in table_rows:
            fqn_parts = [p for p in [row["database_name"], row["schema_name"], row["table_name"]] if p]
            tables.append({
        ...
        col_rows = self._conn.execute(
            col_sql, params + wh_params + [limit]
        ).fetchall()

        columns = []
        for row in col_rows:
            fqn_parts = [p for p in [row["database_name"], row["schema_name"], row["table_name"], row["column_name"]] if p]
            columns.append({
                "warehouse": row["warehouse"],
                "database": row["database_name"],
                "schema": row["schema_name"],
    ...
    def cache_status(self) -> dict[str, Any]:
        ...
        total_tables = self._conn.execute("SELECT COUNT(*) as cnt FROM tables_cache").fetchone()["cnt"]
        total_columns = self._conn.execute("SELECT COUNT(*) as cnt FROM columns_cache").fetchone()["cnt"]
    ...
    def _tokenize_query(self, query: str) -> list[str]:
        ...
        stop_words = {
            ...
            "find", "show", "get", "list", "tables", "columns", "fields",
            "table", "column", "field", "schema", "database",
            "all", "any", "info", "information", "data",
        }
        # Tokenize
