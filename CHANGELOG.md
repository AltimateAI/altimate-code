...
- **`/workspace` menu** — status, refresh the binding, sync memories and skills now, or unlink. Unlink is safe against a relink that lands mid-request and confirms with the server before clearing local state. (#1278)
...
- **`skill publish` is registered only under the workspace pilot**, like `link`; outside it, the command told users to run a `link` command that did not exist for them. Found in this release's review.
