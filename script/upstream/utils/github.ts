...
export async function fetchReleases(
  ...
  const jqFilter = includePrerelease
    ? `.[0:${limit}] | [.[] | select(.draft == false)]`
    : `.[0:${limit}] | [.[] | select(.draft == false and .prerelease == false)]`

  const cmd = `gh api repos/${repo}/releases --paginate --jq '${jqFilter}'`
